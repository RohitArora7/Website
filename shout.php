 ...
 $result2=mysqli_query($con,"select pname from outtjob where id=$rrjobid  and  active=1");
    $row2  = mysqli_fetch_array($result2);  
     ...
     if(is_array($row2))
     {
		...
		if(mysqli_query($con,"INSERT INTO messagejob(jobid, jobpost,jobtemp ,jobneed ,jobneedto ,message,status2) value('$rrjobid','$rrkisnepost', '$rrkonapply' , '$rrkonapply', '$rrkiskoapply', '$message',1 )"))
		{
			
          mysqli_query($con,"UPDATE messagejob SET status2=1 WHERE jobid=$rrjobid and jobneed=$rrkiskoapply and jobneedto=$rrkonapply"); //neutral notifications
	...
	elseif($_POST["fetch"]==1)
	{
		...
		$sql="select messagejob.*,outt.firstname as 'sender' from messagejob join outt on messagejob.jobneed=outt.id where ((jobneed='".$_SESSION['fulljobkon']."' or jobneed='".$_SESSION['fulljobkis']."') and (jobneedto='".$_SESSION['fulljobkis']."' or jobneedto='".$_SESSION['fulljobkon']."')) and  jobid='".$_SESSION['fulljob']."' order by messagejob.id ";
